...
def main():
  ...
  Records_Display = int(
    input(f"Number to Display (Default {Records_Display})? ")
    or f"{Records_Display}")  # How many to display?
  os.system('clear')

  Records_Add = int(
    input(f"Number to Add (Default {Records_Add})? ")
    or f"{Records_Add}")  # How many to Add?
  os.system('clear')

  Records_Remove = int(
    input(f"Number to Remove (Default {Records_Remove})? ")
    or f"{Records_Remove}")  # How many to remove?
  os.system('clear')
  ...
  end_time = time.time()
  total_time = end_time - start_time
  print("Seconds to add", str(Records_Add), "records, Display",
        str(Records_Display), "records, and Remove", str(Records_Remove),
        "Records: {:.6f}".format(total_time))
  Continue()
  print(
    f"Seconds to add {Records_Add} records, Display {Records_Display} records, and remove {Records_Remove} records: {TTime(start_time, end_time)}"
  )
