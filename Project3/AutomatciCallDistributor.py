...
def main():
  ...
  reps = int(
    input(
      f"How many representatives do you want to simulate? (Default {reps}) ")
    or f"{reps}")
  os.system('clear')
  sleeptime = int(
    input(f"How long between simulated seconds? (Default {sleeptime}) ")
    or f"{sleeptime}")
  os.system('clear')
  seconds = int(
    input(f"How many seconds do you want to simulate? (Default {seconds}) ")
    or f"{seconds}")
  os.system('clear')
  answer = int(
    input(f"Random number max, higher = larger queue (Default {rand}) ")
    or f"{rand}")
  os.system('clear')
  min = int(
    input(f"Random Number min, 1 or lower = less queue. (Default {min}) ")
    or f"{min}")
  ...
  print(
    f"Running simulation {seconds} times with spacing of {sleeptime} seconds and a random number maxing out at {rand} and {reps} representaive(s) on the phones."
  )
  rand = rand + reps - 1
  num = 1 + reps
