...
def main():
  ...
  file = int((input(f"Select number of records to sort? (Default {size[0]}) ")
              or "0"))
  os.system('clear')
  ...
  Open(CSVFile[file], clients, Client)
  ...
  Sorting[selection].sort(clients)
  ...
  print(
    f"Seconds to sort {num_records} records: {TTime(start_time, end_time)} using {Sorting[selection]} sorting method"
  )
  Continue()
