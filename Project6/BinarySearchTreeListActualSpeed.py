...
def main():
  ...
  sec_title = ["Scenario 1: Printer Queue/Call Queue/Service Queue", "Scenario 2: Customer Service Center", "Scenario 3: Add/Remove/Delete Random Records"]
  smallest_id = 100001
  rec_disp = 1000
  rec_add = 1000
  rec_del = 1000
  ...
  rec_disp = int((input(f"Number of records to display? (Default {rec_disp}) ") or rec_disp))
  os.system('clear')

  print ("How many records do you want to Add?")
  rec_add = int((input(f"Number of records to Add? (Default {rec_add}) ") or rec_add))
  os.system('clear')

  print ("How many records do you want to Delete?")
  rec_del = int((input(f"Number of records to Delete? (Default {rec_del}) ") or rec_del))
  os.system('clear')

  answer = input(f"Display Records (N/y) (Default {print_records})? " or answer) 
  if answer.lower() == "y":
    print_records = True
  os.system('clear')
  ...
  end_time = time.time()
  print(f"Seconds to display {rec_disp} random records: {TTime(start_time, end_time)}")
